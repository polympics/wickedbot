...
module.exports = {
  ...
  execute: async function(message, argsStr, embed) {
    ...
    const sql1Malus = 'SELECT SUM(malus) AS malus, player_id FROM set INNER JOIN points ON set_id = id WHERE player_id = $1 AND set_id = $2 GROUP BY player_id'
    const values1Malus = [player1.id, setId]
    ...
    const sql2Malus = 'SELECT SUM(malus) AS malus, player_id FROM set INNER JOIN points ON set_id = id WHERE player_id = $1 AND set_id = $2 GROUP BY player_id'
    const values2Malus = [player2.id, setId]
    ...
    try {
      ...
      if (ressel.rows[0].completed && !message.member.roles.cache.has('829548653877657630'))
